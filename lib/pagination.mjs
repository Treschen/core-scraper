...
export async function getProductLinksOnPage(page) {
  ...
  // Give the page a moment to hydrate / render collection cards
  await page.waitForTimeout(2500).catch(() => {});
  await page.waitForLoadState("networkidle").catch(() => {});

  // Try a set of common Shopify/product-grid selectors first
  ...
  // Fallback 1: grab all product-ish anchors, visible or not, but sanitize hard
  const allHrefs = await page.locator('a[href*="/products/"]').evaluateAll((els) =>
    els.map((el) => ({
      href: el.getAttribute("href") || "",
      text: String(el.textContent || "").replace(/\s+/g, " ").trim().toLowerCase(),
    }))
  ).catch(() => []);
  ...
  // Fallback 2: inspect JSON scripts for handles / URLs
