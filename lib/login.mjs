...
function normalizeCookies(raw) {
  ...
  return raw.map((c) => ({
    name: c.name,
    value: c.value,
    // hostOnly cookies must stay without leading dot (exact domain match only)
    // non-hostOnly cookies get a leading dot so subdomains also match
    domain: c.hostOnly ? c.domain : (c.domain.startsWith(".") ? c.domain : `.${c.domain}`),
    path: c.path || "/",
